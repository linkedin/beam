 ...
 *
 * <p>Multi-output counterpart to {@link ParDoWithDlqSpec}. Mirrors the relationship between {@link
 * ParDo.SingleOutput} and {@link ParDo.MultiOutput}: a {@code ParDo.MultiOutput} can be wrapped
 * with {@code .withDlq(DlqSink)} to route per-element user-code throws to the sink while still
 * emitting successful elements to each of the configured output tags.
 *
 * <p>This interface allows the Flink runner (which lives in a different package) to inspect
 * transform properties — {@link #getFn()}, {@link #getMainOutputTag()}, {@link
 * #getAdditionalOutputTags()}, {@link #getSideInputs()}, {@link #getDlqSink()}, {@link
 * #getDlqFilter()} — and reference the {@link #URN} constant, without needing to import the
 ...
 * only through {@link ParDo.MultiOutput#withDlq(DlqSink)} and {@link
 * ParDo.MultiOutput#withDlq(DlqSink, SerializableFunction)}.
 *
...
public interface ParDoWithDlqMultiOutputSpec<InputT> {
  ...
  /**
   * Returns the side inputs threaded through from the wrapped {@code ParDo.MultiOutput} (an empty
   * map when none were declared).
   */
