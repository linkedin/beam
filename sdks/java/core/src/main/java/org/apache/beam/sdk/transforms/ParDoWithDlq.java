 ...
 *
 * <p>Instances are created exclusively via {@link ParDo.SingleOutput#withDlq(DlqSink)} or {@link
 * ParDo.SingleOutput#withDlq(DlqSink, SerializableFunction)}. Callers outside this package interact
 * with this transform through the {@link ParDoWithDlqSpec} interface.
 *
...
class ParDoWithDlq<InputT, OutputT>
   ...
   * handled entirely within the Flink runner via the registered {@link DlqSink} — no side output is
   * added to the Beam pipeline graph.
   *
   * <p>Mirrors {@link ParDo.SingleOutput#expand}'s schema/coder inference so the wrapping does not
   * erase output type info. Without this, callers that wrap an anonymous-class DoFn would see
   * "Unable to return a default Coder" downstream because plain ParDo's coder inference is bypassed
   * by the wrapping PTransform.
   */
  @Override
