 ...
 *
 * <p>Multi-output counterpart to {@link ParDoWithDlq}. Instances are created exclusively via {@link
 * ParDo.MultiOutput#withDlq(DlqSink)} or {@link ParDo.MultiOutput#withDlq(DlqSink,
 * SerializableFunction)}. Callers outside this package interact with this transform through the
 * {@link ParDoWithDlqMultiOutputSpec} interface.
 *
 * <p>The Beam pipeline graph still exposes the same {@link PCollectionTuple} the wrapped {@code
 * ParDo.MultiOutput} would produce — the DLQ side output is handled entirely within the Flink
 * runner via the registered {@link DlqSink} and is not visible in the user-facing tuple.
 *
...
class ParDoWithDlqMultiOutput<InputT>
   ...
   *
   * <p>Sets coders on each output via the {@code CoderRegistry} (same logic as {@code
   * ParDo.MultiOutput.expand}) so that wrapping a {@code ParDo.MultiOutput} with {@code
   * .withDlq(...)} does not erase coder inference — anonymous-class DoFns continue to have their
   * output types resolved automatically.
   */
  @Override
