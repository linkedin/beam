...
//  private static final Logger LOG = LoggerFactory.getLogger(SamzaOutputGBKMetricOp.class);
  ...
  public void processElement(WindowedValue<T> inputElement, OpEmitter<T> emitter) {
    ...
    try {
      for (BoundedWindow x : inputElement.getWindows()) {
        ...
        sumOfCountPerWindowId.put(x, sumOfCountPerWindowId.getOrDefault(x, 0L) + 1);
      }
  ...
  public void processWatermark(Instant watermark, OpEmitter<T> emitter) {
    sumOfTimestampsPerWindowId.forEach(
        (wv, sum) -> {
          System.out.println(String.format("HERE [Output GBK] Window-id=%s for maxTimestamp=%s watermark=%s for task=%s", wv.hashCode(), wv.maxTimestamp().getMillis(),
              watermark.getMillis(), taskContext.getTaskModel().getTaskName()));
          if (wv.maxTimestamp().getMillis() < watermark.getMillis()) {
            System.out.println(
                String.format("HERE [Output GBK] Window-id=%s for sum=%s count=%s watermark=%s for task=%s", wv.hashCode(), sum, sumOfCountPerWindowId.get(wv),
                    watermark.getMillis(), taskContext.getTaskModel().getTaskName()));
